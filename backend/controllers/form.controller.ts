      ...
      await Promise.all(
        question.options.map(async (option: Option) => {
          return await Option.create({
            ...
            question_id: createdQuestions.find(
              (q: Question) => q.dataValues.title === question.title
            )?.dataValues.id,
          });
        })
      );
...
export async function updateForm(req: Request, res: ExpressResponse) {
  try {
    ...
    const optionsInDB = questionsInDB.map(
      (q: Question) => q.dataValues.options
    );
    ...
    const questionsToDelete = questionsInDB.filter(
      (q: Question) =>
        !questions.find((q2: Question) => q2.id === q.dataValues.id)
    );
    const questionsToCreate = questions.filter(
      (q: Question) =>
        !questionsInDB.find((q2: Question) => q2.dataValues.id === q.id)
    );
    const questionsToUpdate = questions.filter((q: Question) =>
      questionsInDB.find((q2: Question) => q2.dataValues.id === q.id)
    );
    ...
    for (const question of questionsToUpdate) {
      ...
      const dbOptionsForCurrentQuestion = flatOptionsInDB.filter(
        (opt: Option) => opt.dataValues.question_id === question.id
      );

      const payloadOptionsIds = new Set(
        (question.options ?? []).map((opt: any) => opt.id)
      );

      const optionsToDelete = dbOptionsForCurrentQuestion.filter(
        (opt: Option) => !payloadOptionsIds.has(opt.dataValues.id)
      );
      ...
      for (const option of question.options) {
        // Update options
        if (
          flatOptionsInDB.find(
            (opt: Option) =>
              opt.dataValues.id === option.id &&
